...
TEST(VertexPackingTests, TestA) {
    SpireVoxel::VertexData vertex = SpireVoxel::PackVertexData(5, 1, 14, 50, 15, SPIRE_VOXEL_VERTEX_POSITION_ZERO, 3);
    glm::uvec3 pos = SpireVoxel::UnpackVertexDataXYZ(vertex.Packed_7X7Y7Z2VertPos3Face);
    EXPECT_EQ(pos.x, 14);
    EXPECT_EQ(pos.y, 50);
    EXPECT_EQ(pos.z, 15);
    EXPECT_EQ(SpireVoxel::UnpackVertexDataVertexPosition(vertex.Packed_7X7Y7Z2VertPos3Face), SPIRE_VOXEL_VERTEX_POSITION_ZERO);
...
TEST(VertexPackingTests, TestB) {
    SpireVoxel::VertexData vertex = SpireVoxel::PackVertexData(63, 19, 63, 64, 0, SPIRE_VOXEL_VERTEX_POSITION_THREE, 0);
    glm::uvec3 pos = SpireVoxel::UnpackVertexDataXYZ(vertex.Packed_7X7Y7Z2VertPos3Face);
    EXPECT_EQ(pos.x, 63);
    EXPECT_EQ(pos.y, 64);
    EXPECT_EQ(pos.z, 0);
    EXPECT_EQ(SpireVoxel::UnpackVertexDataVertexPosition(vertex.Packed_7X7Y7Z2VertPos3Face), SPIRE_VOXEL_VERTEX_POSITION_THREE);
