...
namespace Spire
{
        ...
        : m_surface(surface)
    {
        ...
        if (res != VK_SUCCESS)
        {
            error("Failed to enumerate vulkan physical devices. Result: {}", static_cast<int>(res));
            return;
        }
        ...
        if (res != VK_SUCCESS)
        {
            error("Failed to enumerate vulkan physical devices ({} known physical devices) (result: {})", numDevices,  static_cast<int>(res));
            return;
        }
    ...
    const PhysicalDevice& RenderingDeviceManager::Selected() const
    {
        ...
        assert(m_deviceIndex>=0 && "No device selected!");
        assert(m_deviceIndex<m_devices.size() && "Device index out of bounds!");
